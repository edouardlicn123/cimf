...
def init_database(
    ...
    # 导入数据库工具
    from cimf_django.database import database_exists, drop_database, get_database_config  # noqa: PLC0415
    ...
    if db_exists:
        if reset_db:
            ...
            from core.init_scripts.common import ask_reset_mode  # noqa: PLC0415
    ...
    if stage is None or stage == 1:
        print(colored("\n[1/4] 阶段1：数据库结构", "cyan"))
        run_stage1(skip_migrate=skip_migrate, incremental=incremental, db_exists=db_exists, dry_run=dry_run)
    else:
...
新增：--stage 参数支持单独执行某个阶段
    ...
    parser.add_argument("--force", action="store_true", help="强制重置 admin 用户")
    parser.add_argument("--dry-run", action="store_true", help="模拟执行，不实际写入数据库")
    parser.add_argument("--reset-db", action="store_true", help="删除原有数据库后重新初始化（会丢失所有数据）")
    parser.add_argument("--incremental", action="store_true", help="增量式初始化（保留现有数据，仅更新结构）")
    parser.add_argument("--skip-migrate", action="store_true", help="跳过 migrations（外部已执行时使用）")
    parser.add_argument("--stage", type=int, choices=[1, 2, 3, 4], help="指定初始化阶段（1-4），不指定则执行全部")
    ...
    try:
        ...
        print(colored(f"初始化过程中发生未预期错误: {e!s}", "red"), file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)

...
from core.models import ChinaRegion, SystemSetting, Taxonomy, TaxonomyItem
...
class UserModelTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testuser", password="test12345678", email="test@example.com", role="manager", is_admin=False
        )
    ...
    def test_admin_user(self):
        admin = User.objects.create_user(username="admin", password="admin123456", role="manager", is_admin=True)
        self.assertTrue(admin.is_admin)


class TaxonomyModelTestCase(TestCase):
    def setUp(self):
        self.taxonomy = Taxonomy.objects.create(name="测试分类", slug="test_tax", description="测试描述")
    ...
    def test_taxonomy_items_relationship(self):
        item = TaxonomyItem.objects.create(taxonomy=self.taxonomy, name="选项1", weight=1)
...
class SystemSettingModelTestCase(TestCase):
    def setUp(self):
        self.setting = SystemSetting.objects.create(key="test_key", value="test_value", description="测试设置")

...
from core.models import ChinaRegion, Taxonomy, TaxonomyItem
...
class PermissionServiceTestCase(TestCase):
    def setUp(self):
        self.admin = User.objects.create_user(username="admin", password="admin123456", role="manager", is_admin=True)
        self.manager = User.objects.create_user(
            username="manager", password="manager123456", role="manager", is_admin=False
        )
        self.leader = User.objects.create_user(
            username="leader", password="leader123456", role="leader", is_admin=False
        )
        self.employee = User.objects.create_user(
            username="employee", password="employee123456", role="employee", is_admin=False
        )
...
class TaxonomyServiceTestCase(TestCase):
    def setUp(self):
        self.taxonomy = Taxonomy.objects.create(name="测试分类", slug="test_category", description="测试用分类")
        self.item1 = TaxonomyItem.objects.create(taxonomy=self.taxonomy, name="选项一", weight=1)
        self.item2 = TaxonomyItem.objects.create(taxonomy=self.taxonomy, name="选项二", weight=2)
    ...
    def test_update_taxonomy(self):
        updated = TaxonomyService.update_taxonomy(self.taxonomy.id, name="更新后的分类")
    ...
    def test_create_item(self):
        new_item = TaxonomyService.create_item(self.taxonomy.id, "新选项", description="描述")
        self.assertEqual(new_item.name, "新选项")
        self.assertEqual(new_item.taxonomy_id, self.taxonomy.id)

    def test_update_item(self):
        updated = TaxonomyService.update_item(self.item1.id, name="更新后的选项")
...
class ChinaRegionServiceTestCase(TestCase):
    def setUp(self):
        self.province = ChinaRegion.objects.create(code="440000", name="广东省", level=1)
        self.city = ChinaRegion.objects.create(code="440100", name="广州市", level=2, parent=self.province)
        self.district = ChinaRegion.objects.create(code="440105", name="天河区", level=3, parent=self.city)
...
class WatermarkServiceTestCase(TestCase):
    def test_add_text_watermark_returns_bool(self):
        ...
        try:
            result = WatermarkService.add_text_watermark(in_path, out_path, "测试水印", position="center", opacity=0.3)
    ...
    def test_add_text_watermark_invalid_path(self):
        result = WatermarkService.add_text_watermark("/nonexistent/image.png", "/tmp/output.png", "Test")
        self.assertFalse(result)

    def test_add_image_watermark_invalid_image(self):
        result = WatermarkService.add_image_watermark(
            "/nonexistent/image.png", "/tmp/output.png", "/nonexistent/logo.png"
        )
        self.assertFalse(result)
...
class UserServiceTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="testuser", password="test12345678", role="employee", is_admin=False
        )

    def test_create_user_empty_password(self):
        with self.assertRaises(ValueError) as context:
            UserService.create_user(
                username="newuser", nickname="New User", email="new@example.com", password="", role="employee"
            )
    ...
    def test_create_user_short_password(self):
        with self.assertRaises(ValueError) as context:
            UserService.create_user(
                username="newuser", nickname="New User", email="new@example.com", password="short", role="employee"
            )

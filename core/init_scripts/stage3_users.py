...
功能说明：
    ...
    支持以下特性：
    - 生产环境密码强度检查（≥10位）
    - 通过环境变量配置管理员信息（DJANGO_ADMIN_USERNAME 等）
    - --force 参数强制重置已存在的管理员
    - 生产环境保护限制（需要 DJANGO_ALLOW_SEED_PROD=1）
...
环境变量：
    ...
    - DJANGO_ADMIN_NOTIFICATIONS: 是否启用通知（默认: true）
    - DJANGO_ADMIN_PREFERRED_LANGUAGE: 偏好语言（默认: zh）
    - DJANGO_ALLOW_SEED_PROD: 允许生产环境执行（默认: false）

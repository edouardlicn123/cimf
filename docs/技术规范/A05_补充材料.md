...
> 本文档为 `A05_Python代码开发规范.md` 的补充材料，包含完整的代码示例、模板和详细说明。
...
**服务层（`core/services/mixins.py`）：**

```python
def success_response(**kwargs):
    """成功响应，返回 dict（非 HTTP 响应）"""
    return {"success": True, **kwargs}


def error_response(message, **kwargs):
    """错误响应，返回 dict（非 HTTP 响应）"""
...
# 服务层示例
def check_and_return(user):
    if not user.is_authenticated:
        return error_response("需要登录")
...
# 注意：Jinja2 默认 autoescape=True，再次添加 |escape 会导致双重转义
# Jinja2 中 {{ user_input }} 已是安全的，不应额外加 |escape

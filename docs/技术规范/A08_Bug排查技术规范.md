...
> 以下统计数据反映历史修复前的状态。截至目前，文档中引用的所有 Bug 已全部修复。

| 严重程度 | 数量 | 占比 | 典型问题 |
|----------|------|------|----------|
| 🔴 P0（安全/崩溃） | 已清零 | — | f-string注入、开放重定向、定时任务仅执行一次、for-else缩进、JS语法错误 |
| 🟠 P1（功能异常） | 已清零 | — | 装饰器缺失、.first()未检查、模板语法错误、CSRF暴露 |
| 🟡 P2（代码质量） | 已清零 | — | 死代码、多余导入、logger使用不当 |

| 层级 | Bug数 | 主要问题 |
|------|-------|----------|
| 服务层 | 已清零 | `.first()` None检查、`datetime.now()`、`except:pass` |
| 视图层 | 已清零 | 装饰器缺失、CSRF处理、参数验证 |
| 模板层 | 已清零 | Jinja2语法混用、csrf_token语法、外键None |
| 模块系统 | 已清零 | 动态加载安全、f-string注入、死代码 |
| 配置层 | 已清零 | 环境变量不匹配、APP_DIRS、WAL |
| 导入导出 | 已清零 | CSV注入、编码崩溃、key不匹配 |
...
> ⚠️ 以下所有 grep 命令为历史排查命令，当前代码库中对应问题已全部修复。
...
| # | 检查项 | 检查命令 |
|---|--------|----------|
| 1 | `__import__` 前验证模块安装状态 | `grep -rn "__import__" core/ modules/`（当前结果为空，已全部清理） |

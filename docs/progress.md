...
26. 修正customer_cn模块模板为entry-card样式
27. 修复customer_cn模块模板路径和目录结构
28. 修复乱码（禁用jinja2.ext.debug）和省市县联动（导入3429条省市区数据）
29. 实现省市县数据从网络下载更新本地文件的完整逻辑
30. 修复客户编辑页乱码：'所���行业' → '所在行业'
31. 修复 CSRF token 显示问题：将 Jinja2 模板中的 '{{ csrf_token }}' 改为 '{% csrf_token %}'

# 2026-04-28 修改记录

1. 修复 CSRF token 显示：修改 context_processor 返回完整 input 元素，并在 jinja2.py 中注册 csrf_token 函数
2. 更新技术规范文档：A03新增数据维护流程章节、A04新增csrf_token实现机制说明
3. 更新规范文档：B02新增ChinaRegionService方法、A04新增Jinja2环境配置章节和过滤器速查
4. 更新A01项目概述文档：删除customer_cn/resident_info，新增calc模块，更新文档列表
5. 更新A02模块技术规范：删除customer_cn/resident_info，添加calc目录结构
6. 更新A04模板开发规范：删除customer_cn/resident_info相关示例
7. 修复A03省市县规范：删除重复行，修正章节编号
8. 全面修复技术规范文档：A01/A03/A06添加版本和日期，B01-B05/A06补充换行符
9. 创建 customer_cn 模块技术规范文档（modules/customer_cn/docs/README.md）
10. 补充 customer_cn 模块技术文档：新增分页参数、表单处理流程、API响应格式、表单验证与错误处理章节
11. 修复登录页 CSRF token 嵌套问题
12. 检查并修复所有模板的 CSRF token 使用方式
13. 修改 entry_card 样式：标题移至照片区域下方，图标透明度改为 70%
14. 模块市场 customer_cn 升级到 v1.1.5
...
3. 第五轮审查修正7份技术规范文档：A01修正模板目录树结构（删除不存在的core/嵌套层和modules/目录）、修正核心模块字段数24→26；B03修正logs视图计数3→2、node/views计数~15→14、taxonomy_items_api归类、补充导入导出视图来源；B02补充SMTP服务（SmtpService/EmailService/TemplateService/定时任务）、修正base_service为抽象基类不导出；A05修正nodes.models→core.node.models、makemigrations示例更新；A04全面重写目录结构章节删除不存在的nodes/目录按实际文件列表更新；B01完善模型关系图补充Module-NodeType-ToolType关联；B05标注taxonomy_items_api视图来源为core/node/views.py
4. 第六轮：修正 B03 logs(3)/cron(8)/tools(4) 视图计数、更新 module_dispatch 分发规则；修正 A02 core/node/ 目录树；修正 B05 分发优先级

# 2026-05-06 修改记录

1. 修正B06词汇表数量：37→47（fixture实际包含47个词汇表）；开发规范.md模板目录已确认完整
2. 全面审查并修正技术规范文档：B02修正task位置/删除重复LogService/词汇表47；B03修正视图计数；B05修正API路由数17/importexport namespace来源；B01补充User继承AbstractUser说明/模型总数；A01补充tasks目录；A05修正测试目录结构/jinja2.py
3. 第十轮文档审查：B03修正calc/smtptest视图数(各1→2)；B04修正UserCreateForm字段(移除is_active)；B02设置默认值确认准确；确认logs_api为未使用死代码
4. 修复3个潜在代码问题：(1) 删除taxonomy_service.py中DEFAULT_TAXONOMIES回退方法，简化init_default_taxonomies()为纯fixture加载；(2) logs_api添加TODO注释标注未注册路由；(3) core/views.py保留不动。同步更新B06文档(v1.4)
5. 第十一轮文档审查：B02修正VersionService实际方法(6个非3个)/SampleDataService实际方法(仅init_sample_customers)/修正Section编号(13.x→12.x, 14.2→13.2)/修正tasks基类名CronTask(v1.8)；A04修正第一个目录树(admin/node/errors/includes缺失文件)/更新版本号v2.5
6. 第十二轮文档审查：A04修正第一个目录树(admin/node/errors/includes缺失文件)/更新版本号v2.5；B03修正@admin_required行为描述(重定向非返回JSON)/更新v1.8；B02修正tasks基类名CronTask(v1.8)
7. 第十三轮文档审查：B03标注smtp_presets为未注册URL预留功能(v1.9)
8. 第十四轮文档审查：B02补全SettingsService默认配置(15→51项/v1.9)；B01补全Module模型缺失字段(icon, install_on_init/v1.5)
9. 第十五轮文档审查：B05修正API路由计数回19（此前误改为17）/v1.7
10. 第十六轮文档审查：修正B02文档版本号(头部1.7→1.9，保持与尾部一致)
11. 第十七轮文档审查：修正A01/A02脚部版本号(A01:1.5→1.7, A02:3.4→3.6)；全面验证所有文档版本一致性；所有12份技术规范文档版本号已确认一致
12. 全面审查设计规范文档，修正12处不一致：B04表单继承类Form→ModelForm、B06 FAQ引用DEFAULT_TAXONOMIES→fixture、B05路由计数42→43、A02模块引用路径修正、B02补充LogService、A04继承图补充3个frame、A01功能描述修正、开发规范版本号修正
13. 第5轮审查：修正B03遗漏3个API路由(health/detailed/version)、修复cards.py缺失logging导入
14. Round2: 修复core/fields/__init__.py文档注释(24→26种+补充region_select)、删除core/node/services/module_service.py重复文件、修正B05根URL配置代码示例(v1.8→v1.9)
15. Round4-5: 修正B06模块服务路径(core/node→core/module)、全局交叉验证完成(版本号/路由计数/字段数/模型数/服务签名/表单定义/常量/目录结构全部一致)
16. Round2-审查修复9项: A01修正models继承描述、A02修正Node FK用settings.AUTH_USER_MODEL+补frontpage_card_clickable、B01补preferred_language字段、B02修正Cron任务间隔表+导出LogService、B03修正module_dispatch代码、B05/B06前轮修复
17. 全面检查设计规范文档5次（B01-B05），自动修复：B01章节编号、B02补充服务信息、B03修正API视图数量、B04补充mixins.py、B05验证路由数量
18. 5次全面检查B01-B05文档：修复拼写错误（nick_name→nickname等）、语法错误（cleaned_data→cleaned_data等）、章节编号错误（EmailLog章节）、版本号更新（B01→1.9, B02→2.4, B03→2.3, B04→1.5, B05→2.0）
19. 修复导入错误显示undefined问题：移除import_page.html中对不存在的err.field的引用
20. 修改导入导出图标：导入改为bi-database-up，导出改为bi-database-down
21. 修复导入错误显示undefined：统一upload_preview错误返回格式，添加message字段

# 2026-05-07 修改记录

1. 修复module_scan错误：调用scan_register_install替代scan_and_register_modules，使用正确的返回值字典
2. 新增frontpage_card参数控制首页功能卡片显示：后端逻辑修改，更新clock/customer模块为True，calc模块为False

# 2026-05-08 修改记录

1. 完善smtptest模块：添加邮件发送页面(/tools/smtptest/)，实现tool_view发送5封测试邮件，调用后台SMTP系统
2. 修复smtptest模块bug：移除未使用导入，添加邮箱格式验证，优化错误处理
3. 修复smtptest模块bug：修正f-string语法错误、邮箱正则转义问题
4. 修复 smtptest 模板 Jinja2 语法错误 (join 过滤器使用括号语法)
5. 修复 smtp 模块多处 bug: config.html 表单值丢失/按钮404/嵌套表单, smtptest views 变量名, 移除死代码 smtp_presets
6. 修复 smtptest 模块 bug: 验证失败保留输入/SMTP禁用提示, save_config 更新 Django 设置, 密码为空错误信息破坏, history 过滤改到 DB 层
7. 修复模板渲染(改用Jinja2)、test_connection SSL支持、sent_count计数、失败通知from_email回退、smtptest结果页回传输入
8. smtptest 图标改为空心邮件(bi-envelope)
9. 路由规范: 按模块类型分配前缀 (node→nodes/, tool→tools/, system→system/), 更新 modules/urls.py/customer views/API/JS/文档
10. 全面Bug修复: S1开放重定向/for-else缩进/模板继承路径/card_clickable, S2 EmailLog类型/csrf_token/logs int()/calc __init__, S3 smtp_config权限/Jinja2渲染安全/node自定义URL重定向修复, S4死代码清理/冗余装饰器移除
11. 第二轮修复: S1 Http404/@require_POST/paginator括号/datetime→timezone/watermark, S2 except:pass加日志/csrf_exempt移除/setInterval/BroadcastChannel去重, S3死代码标注
12. 第三轮Bug修复: P0修复cron定时任务仅执行一次/is_admin覆盖role; P1修复JsonResponse崩溃/环境变量名不匹配/CSV注入/编码OOM/Zip bomb SSRF/线程锁/get_status权限; 共修复12项安全+功能bug
13. 第三轮全面Bug检查+修复: P0修复dashboard_cards_area JS闭括号; P1修复cron缺少require_POST/cron_service缺datetime导入/用户搜索icontains+%冲突/module_create参数错误/module_dispatch缺少安全验证/CSRF暴露/download_errors死功能/ZIP Slip绕过/农历计算优先级/calc eval安全/IP中间件HTML响应; 共修复P0x1+P1x12项
14. 模块全面检查：完成 modules/ 下 calc/customer/clock/smtptest 四个模块的全面代码审查，发现 12 个问题（1个P1/11个P2），生成详细报告
15. 第四轮全面Bug检查+修复: P0修复create_module f-string代码注入; P1修复importexport views.py json未导入+key不匹配/CSV注入\t无效/导出过滤器前缀错误/import_service跳过False/模板导入冲突/services.py自引用/DjangoTemplates APP_DIRS/WAL模式/过滤器前缀错误/module_create响应不一致; 共修复P0x1+P1x10项
16. 合并bug排查规范到A08_Bug排查技术规范，删除旧文件，更新AGENTS.md引用
17. 全面Bug修复: 删除node/services.py导入冲突, datetime.now→timezone.now, 7个模板extra_head→head_extra, market_install admin权限, console.log清理, print→logger(3个service), 删除空urls.py, 移除include中无效block
18. Serverworks风格设计细节优化: 按钮填充动画btn-fill+药丸形btn-pill+微抬升+按下反馈, 卡片hover上移+图片缩放, 标题letter-spacing, 统一焦点环, 图标动画替换, 过渡曲线统一cubic-bezier
19. Entry-card 按钮：药丸形 + 描边 + per-variant 填充动画（frame.css，适配全部 7 主题）
20. Entry-card 按钮 hover 文字色改为 white（--text-inverse 未在任何主题中赋值）
21. 模块列表页搜索按钮改为圆形图标按钮（删除文字，新增 .btn-circle）
22. 模块管理页搜索按钮改为圆形图标按钮
23. Serverworks 风格页面标签 + 卡片圆角统一为 radius-sm (7 个 frame, 2 个 CSS)
24. 回滚 7 个 frame 模板（恢复 h1 标题），删除 .page-tag CSS，新增 card-header 梯形标签（clip-path 上窄下宽 + 绝对定位浮动在卡片上方）
25. 修复 card-header 梯形标签被 overflow 裁切：改用 position: relative + top 替代 absolute
26. 重写 card-header 梯形标签：抹掉 card-header 样式(border/bg/padding)，梯形底部紧贴 card-body，匹配 Serverworks After 卡片设计
27. card-header 梯形 absolute 浮动在卡片上方，card 透明 + card-body 持白色背景，clip-path 外透明区直接透出页面背景
28. 回退 absolute 方案，梯形改用 inline-block 在卡内顶部，clip-path 外透明区透出 card 白色背景
29. 梯形标签对应 card 透明 + border-radius: 0，card-body 持白色背景
30. 梯形标签卡片去除 box-shadow（消除 card 透明后透出的外发光）
31. 梯形标签卡片去除 border（消除边沿透出）
32. 梯形标签 card-body 重置 border-radius/border/box-shadow/padding（覆盖主题 !important）
33. 梯形标签 card-body 边框改为 1px solid var(--primary) 与梯形同色
34. 梯形标签 card-body 圆角: 左上 0 + 右上/右下/左下 var(--radius-sm)
35. 梯形 clip-path 左上角改为直角 (0 0)
36. 梯形改用 CSS mask + border-radius 6px 实现顶部双圆角
37. 回退 mask 圆角方案，恢复 clip-path 梯形
38. 删除梯形 clip-path，改为顶部圆角(6px)底部直角的标签形状
39. 权限管理页：系统/节点权限卡片改为 h5 标签标题，去掉 p-0，CSS 去掉 padding 强制覆盖
40. 角色名称设置卡片标题前添加 bi-mask 图标

# 2026-05-09 修改记录

1. 创建 D01_CSS外观设计标准套件.md，录入'基本卡片'设计规范；frame.css 注释更新
2. SMTP 邮件配置页：状态/最近发送记录卡片改为基本卡片设计
3. 日志管理页：分级筛选控件移出 card-header 到 card-body，修复基本卡片标题显示
4. 日志管理页：简化卡片标题表达式，按 log_type 显示对应中文日志名
5. 日志管理页：选择文件按钮改为描边药丸样式，选中白字，文件大小标改为橙色
6. 将基本过滤标签设计标准录入 D01_CSS外观设计标准套件.md
7. 权限检测页：三个过滤按钮改为基本过滤标签风格（nav-pills-outline），选中态badge!important适配
8. 基本过滤标签 border-radius 改为 6px 6px 0 0（顶部圆角底部直角）
9. 基本过滤标签选中态badge改为橙色底黑字
10. 日志管理页：合并为单个卡片，文件pills与级别筛选同行，空状态内联
11. 日志表格加主色边框(log-table-wrap)，过滤栏间距缩小至mb-2，选中态badge文字!important黑色
12. 日志表格边框改为直角，间距缩至mb-1紧贴过滤按键
13. 日志过滤栏 align-items-end + margin-bottom:-1px 使边框紧贴过滤标签
14. 日志管理页 logs_index 改为直接跳转 cimf 日志
15. 文档新增第3节'基本列表'：log-table-wrap 紧贴过滤标签的表格边框设计
16. 日志过滤栏去掉负margin，恢复 mb-2 避免遮盖列表
17. 权限检测页：过滤按键移至 card-body 紧贴 log-table-wrap，与日志页一致
18. Cron调度页：列表改为基本列表设计（card-header h5 + log-table-wrap）
19. Cron调度页：去掉 log-table-wrap，仅保留 card-body 主色边框
20. 人员管理页：列表改为基本列表（card-header h5 + log-table-wrap），标题改为'人员管理'
21. 人员管理/cron 页 card-body p-0 去掉内边距和双层边框，列表填满容器
22. 全局字体优化：创建 fonts.css 直连加载 + preload 核心 woff2 + 补充 Inter SemiBold
23. 字体切换修复：font-display:swap → block，消除回退字体闪烁
24. 首页快捷入口/常用链接改为基本卡片设计（card-header h5 + card-body p-0）
25. 基本卡片 card-body 加 overflow:hidden 确保 p-0 时圆角可见
26. 基本按键：添加渐变滑动动画（背景扫描效果） via frame.css 全局覆盖 btn-primary/success/danger/warning/info，登录页用 !important 保护，文档 D01 第4节
27. 基本按键渐变修正：从 rgba(0,0,0,X) 暗化改为 color-mix(white 20%) 浅色变体，实现 浅色→主色→浅色 对称渐变，hover反向扫描
28. 基本按键渐变修复：background→background-image 防止重置 size/position，三色渐变改为两色 lighter→primary
29. 基本按键渐变修复：两色渐变加中间停点 primary 50%，确保 200% size 下各可见半窗内完成完整 lighter→primary 过渡
30. 基本按键主色占比调整：渐变停点从 50% 改为 17% 83% 双位置，主色占可见区 2/3
31. 基本按键停点改为 30% 70%，主色占可见区 80%
32. 基本按键渐变方向：to right → 135deg（顺时针旋转45度）
33. 多巴胺主题配色替换为 Serverworks 品牌色：--primary #D00F31 + --info #FB8E22，保留圆角/过渡个性
34. 模块管理页搜索栏布局调整：搜索按钮嵌入搜索框右侧（input-group），筛选列均分剩余空间
